...
cmd:option('-log_return_offsets',"10",'list of comma separated offset values that should be used to compute additional log return features')
cmd:option('-log_return_ema_period',5,'EMA period for additional log returns')
cmd:option('-warmup_offset',20,'Offset applied at the start of the features tensor before starting the training process')
cmd:option('-forcast_index',2,'Index of the feature that should be forcasted.')
cmd:option('-num_classes',1,'Number of classes to consider when performing classification.')
cmd:option('-label_offset',10,'Offset to apply when generating the labels')

cmd:option('-num_networks',1,'Number of networks that should be trained in parallel by this predictor')
cmd:option('-train_frequency',10,'Frequency at which we try to perform a training (in number of samples received)')
...
cmd:option('-max_epochs',15.0,'number of full passes through the training data')
cmd:option('-initial_max_epochs',300.0,'number of full passes through the training data on the first training session')
